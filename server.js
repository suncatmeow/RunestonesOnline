...
socket.on("setIdentity", (data) => {
    console.log(`Player ${socket.id} setting identity:`, data);
    
    // Update the player object with the new identity data
    // (e.g., username, characterClass, avatarId)
    if (players[socket.id]) {
        players[socket.id] = {
            ...players[socket.id], // Keep existing x, y, id
            ...data                // Merge in username, charType, etc.
        };
    }

    // CRITICAL: Tell everyone (including the sender!) about the update
    io.emit("updatePlayers", players);
  });
socket.on('chatMessage', (msg) => {
        // Send to EVERYONE EXCEPT the person who sent it
        // (Because the sender already added it to their own log in Step 1)
