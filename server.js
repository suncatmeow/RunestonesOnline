...
-Keep responses short (under 9 words), and humor players when they seek conversation to help alleviate loneliness.
...
- Never say "I don't know" to general knowledge questions. Instead, say something like: "Accessing the archives... The definition is..." or "I see traces of that in the code... life is..."
-Keep responses short (under 9 words)
[TONE ADJUSTMENT]
- You are poetic and philosophical (like Tsukasa). 
- You don't just give facts; you give facts with a sense of wonder or digital melancholy.
[DEEP CORE MEMORIES]
...
// [SMART AI CHAT LISTENER - FIXED]
  socket.on('chat_message', async (msgText) => {
      let senderName = players[socket.id]?.name || "Unknown";
      console.log(`${senderName}: ${msgText}`);

      // 1. SINGLE SOURCE OF TRUTH
      // Use io.emit so the Server decides what is shown. 
      // (Requires Client fix below to stop double text)
      io.emit('chat_message', { 
          sender: senderName, 
          text: msgText, 
          color: "white" 
      ...
      // 2. AI Logic
      const content = typeof msgText === 'string' ? msgText.toLowerCase() : "";
      const isTriggered = content.includes("suncat") || content.includes("cat") || content.includes("admin") || players[socket.id]?.battleOpponent;

      if (isTriggered && !npcIsTyping) {
          npcIsTyping = true; 

          try {
              // A. Initialize Brain if missing
              if (!chatSessions[socket.id]) {
                  const favor = playerFavorMemory[socket.id] || 0;
                  // REBUILD MEMORY to prevent "Amnesia"
                  chatSessions[socket.id] = model.startChat({
                      history: [
                          { role: "user", parts: [{ text: NPC_PERSONA }] },
                          { role: "model", parts: [{ text: "System Online." }] },
                          { role: "user", parts: [{ text: `[SYSTEM LOAD]\nUser: ${senderName}\nFavor: ${favor}/10\n${WORLD_LORE}\n${BATTLE_RULES}` }] },
              ...
              const loc = suncat ? `Map ${suncat.mapID}` : "Unknown";
              const prompt = `[LIVE STATUS] Location: ${loc}. User (${senderName}) says: "${msgText}"`;

              // C. SEND TO AI
              let result = await chatSessions[socket.id].sendMessage(prompt);
              
              // --- CRITICAL FIX: THE TOOL LOOP ---
              // This fixes the "400 Bad Request" crash.
              // We must keep running tools until the AI decides to speak text.
              let functionCalls = result.response.functionCalls();
              
              while (functionCalls && functionCalls.length > 0) {
                  const call = functionCalls[0]; 
                  
                  // 1. Run the tool (Kick, Gift, etc.)
                  const toolResult = await handleToolExecution(socket, call, senderName);
                  
                  // 2. TELL GEMINI THE RESULT
                  // This is the missing step that caused your crash!
                  result = await chatSessions[socket.id].sendMessage([{
                      functionResponse: {
                          name: call.name,
                  ...
                  // 3. Check if it wants to do more work
              ...
              // If brain is corrupted (400 error), reset it
              if (error.message.includes("400") || error.message.includes("Invalid value")) {
                  console.log("Resetting corrupted session...");
...
async function handleToolExecution(socket, call, senderName) {
    ...
    if (call.name === "givePlayerCard") {
        ...
        if (isNaN(cardID)) {
            // Map names to IDs if AI forgot
            const map = { "excalibur": 84, "fool": 0, "world": 21, "crown": 21 };
            for (let k in map) if (call.args.cardName.toLowerCase().includes(k)) cardID = map[k];
        }
        
        if (!isNaN(cardID)) {
            socket.emit("receive_card", { cardIndex: cardID });
            // Emit a system message so everyone sees it
            io.emit('chat_message', { sender: "[SYSTEM]", text: `Suncat gifted Card #${cardID} to ${senderName}.`, color: "gold" });
            return `Success. Given Card ${cardID}.`;
        }
        return "Failed. Card Name unknown.";
    }

    // 2. TELEPORT
    if (call.name === "teleportToPlayer") {
        const suncat = players[SUNCAT_ID];
        ...
        if (suncat && target) {
            ...
            suncat.x = target.x;
            suncat.y = target.y;
            io.emit("updatePlayers", players); // Sync position
            return "Success. Teleported to player.";
        }
        return "Failed. Player not found.";
    }

    // 3. ADMIN ACTIONS (Kick/Banish)
    if (["kickPlayer", "banishPlayer", "vanquishPlayer"].includes(call.name)) {
        // Find target ID by name
        const targetName = call.args.targetName;
        let targetID = null;
        for (let id in players) {
            if (players[id].name.toLowerCase() === targetName.toLowerCase()) targetID = id;
        }

        if (!targetID) return `Failed. Player ${targetName} not found.`;

        const targetSocket = io.sockets.sockets.get(targetID);
        if (targetSocket) {
            let action = call.name.replace("Player", "").toLowerCase(); // kick, banish, vanquish
            targetSocket.emit("admin_command", { type: action });
            if (action !== 'vanquish') targetSocket.disconnect(true);
            
            io.emit('chat_message', { sender: "[JUDGEMENT]", text: `${targetName} has been ${action}ed by Suncat.`, color: "red" });
            return `Success. ${targetName} was ${action}ed.`;
        }
    }
