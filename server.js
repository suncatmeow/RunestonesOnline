...
// Change the top section to this:
const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);

// Explicitly use 'gemini-1.5-flash-latest' which usually resolves v1 mapping issues
const model = genAI.getGenerativeModel({ 
    model: "gemini-1.5-flash",
    apiVersion: 'v1beta' // Adding this often fixes the 404 Not Found error
});
...
const NPC_PERSONA = "You are Suncat, a mysterious digital wizard trapped in a retro game. You are helpful but slightly glitchy. Keep responses short (under 15 words).";
...
io.on("connection", (socket) => {
  ...
  socket.on('chat_message', async (msgText) => {
      ...
      const randomChance = Math.random() < 0.05; // 5% chance to speak randomly
