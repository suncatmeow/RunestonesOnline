...
io.on("connection", (socket) => {
 ...
 // [SMART AI CHAT LISTENER]
  socket.on('chat_message', async (payload) => {
      ...
      // --- 1. UNPACK THE PAYLOAD (Fixing the crash) ---
      let msgText = "";
      let isContinuation = false;

      // Check if we received an object (New Style) or just text (Old Style)
      if (typeof payload === 'object' && payload.text) {
          msgText = payload.text;        // Extract the actual text
          isContinuation = payload.cont || false; // Extract the flag
      } else {
          msgText = payload;             // It was just a string (Legacy)
      }
      ...
      // --- 3. AI LOGIC ---
      // We use 'msgText' for the AI because 'payload' might be an object
              ...
              setTimeout(async () => {
                  try {
                      ...
                      if (call) {
                        ...
                        // 4. EMIT CHUNKS (With Continuation Flag)
                        chunks.forEach((chunk, index) => {
                            io.emit('chat_message', {
                                sender: NPC_NAME,
                                text: chunk,
                                color: "#00ffff",
                                cont: (index > 0) // TRUE if it's the 2nd, 3rd line...
                            });
                        });
                    }
  ...
  socket.on("challenge_request", (data) => {
    // [NEW] SUNCAT BOSS BATTLE INTERCEPTION
    if (data.targetId === SUNCAT_ID) {
        console.log(`[BATTLE] ${players[socket.id].name} challenged Suncat!`);
        
        // 1. Generate Suncat's Deck (Random or Thematic)
        // Let's give him a chaotic deck (mostly Wands/Fire or Glitch cards)
        let suncatDeck = [];
        for(let i=0; i<30; i++) {
            suncatDeck.push(Math.floor(Math.random() * 78)); 
        }

        // 2. Automatically Accept the Challenge
        socket.emit("challenge_accepted", {
            opponentId: SUNCAT_ID,
            opponentDeck: suncatDeck,
            isNpcBattle: true // Special flag for the client
        });
        return; // Stop here, don't try to send to a socket that doesn't exist
    }
    // 2. Normal PVP Logic
    if (io.sockets.sockets.get(data.targetId)) {
        ...
        console.log("Target socket not found:", data.targetId);
    }
  });
