                      ...
                      if (call) {
                          ...
                          if (call.name === "givePlayerCard") {
                                ...
                                if (!isNaN(cardID)) {
                                    ...
                                    sendChunkedMessage(io, NPC_NAME, completion.response.text(), "#00ffff");
                                } else {
                                     // Handle invalid card
                                    const errorOutput = {
                                        functionResponse: {
                          ...
                          // [REPLACE YOUR EXISTING JUDGEMENT BLOCK WITH THIS]
                                ...
                                if (suncat && requester) {
                                    ...
                                    sendChunkedMessage(io, NPC_NAME, aiText, "#00ffff");
                                    // 4. BROADCAST TO CHAT
...
// --- [UPDATED] SUNCAT BATTLE VOICE ---
  socket.on('suncat_battle_speak', async (data) => {
    // 1. Get the session
    const userSession = chatSessions[socket.id];
    if (!userSession) return;

    try {
        console.log("Suncat generating battle commentary...");

        // 2. Safe Data Handling (Prevent crashes if data is missing)
        const myMonsters = data.myMonsterIDs ? data.myMonsterIDs.join(', ') : "None";
        const oppMonsters = data.oppMonsterIDs ? data.oppMonsterIDs.join(', ') : "None";
        const myHand = data.myHandIDs ? data.myHandIDs.join(', ') : "Hidden";
        const oppHand = data.oppHandIDs ? data.oppHandIDs.join(', ') : "Hidden";

        // 3. Construct the prompt
        const battlePrompt = `
        [SYSTEM EVENT: BATTLE COMMENTARY]
        You are currently in a card duel with this player. 
        Analyze the battle state below and write a SHORT (under 12 words) intense reaction.
        
        [CURRENT BATTLE STATE]
        - Phase: ${data.phase} (5=Planning, 6=Dice Rolling, 7=Result)
        - My Monster ID: ${myMonsters} 
        - Enemy Monster ID: ${oppMonsters} 
        - My Hand IDs: ${myHand}
        - Enemy Hand IDs: ${oppHand}
        - My Roll: ${data.myRoll} vs Enemy Roll: ${data.enemyRoll}
        - Winning Roll?: ${data.winning}
        - Am I Bound/Stunned?: ${data.bound}
        - Is Enemy Bound/Stunned?: ${data.oppBound}
        - My Action Type: ${data.myAction}
        - Enemy Action Type: ${data.oppAction}

        [REFERENCE DATA]
        ${CARD_MANIFEST}
        
        [INSTRUCTIONS]
        - If Phase 5 (Planning): Comment on your strategy or the enemy's weak monster.
        - If Phase 6/7 (Result): 
            - If WINNING: Gloat.
            - If LOSING: Glitch out or threaten them.
        `;

        // 4. Send to AI
        const result = await userSession.sendMessage(battlePrompt); 
        const response = result.response.text();

        // 5. Broadcast
        sendChunkedMessage(io, "Suncat", response, "#00ffff");
       

    } catch (e) {
        console.error("AI Battle Speech Failed", e);
    }
  });
  socket.on("move", (data) => {
    if (players[socket.id]) {
    ...
    if (nearbyPlayer && chatSessions[nearbyPlayer.id]) {
        ...
        setTimeout(async () => {
            try {
                ...
                const response = result.response.text();
                sendChunkedMessage(io, NPC_NAME, response, "#00ffff");
...
// --- HELPER: CHUNK & SEND MESSAGES ---
function sendChunkedMessage(io, sender, text, color) {
    if (!text) return;

    // 1. Clean Tags (Remove [[FAVOR: +1]] etc)
    const cleanText = text.replace(/\[\[.*?\]\]/g, "").trim();
    if (!cleanText) return;

    // 2. Split into Chunks
    const MAX_LEN = 60; // Characters per line
    ...
    words.forEach(word => {
        if ((currentLine + word).length < MAX_LEN) {
            ...
            io.emit('chat_message', { sender, text: currentLine, color });
            currentLine = word;
        }
    });
    
    // Emit the last remaining line
