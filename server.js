             ...
             // --- [NEW] INJECT PLAYER LIST CONTEXT ---
              ...
              setTimeout(async () => {
                  try {
                      ...
                      // --- [NEW] ROBUST TOOL EXECUTION BLOCK ---
                      const call = result.response.functionCalls()?.[0];

                      if (call) {
                          console.log(`[AI TOOL CALL]: ${call.name}`); // Debug log
                          ...
                          // C. TELEPORTATION
                            ...
                            if (suncat && requester) {
                                // 1. EXECUTE THE MOVE (With Number safety!)
                                suncat.mapID = requester.mapID;
                                suncat.x = parseFloat(requester.x); // Force Number
                                suncat.y = parseFloat(requester.y); // Force Number
                            ...
                            io.emit('chat_message', { 
                                sender: NPC_NAME, 
                                text: aiText || "*Suncat appears in a glitch of pixels.*", 
                                color: "#00ffff" 
                            });
                        }

                        // D. [CRITICAL FIX] CATCH-ALL FOR UNKNOWN TOOLS
                        // This prevents the AI from freezing if it calls "teleport" instead of "teleportToPlayer"
                        else {
                            console.log(`Unknown tool called: ${call.name}`);
                            const errorResult = {
                                functionResponse: {
                                    name: call.name,
                                    response: { result: "Error: Function does not exist. Please check your tools definition." }
                                }
                            };
                            // We respond to the AI so it doesn't hang waiting for an answer
                            const completion = await chatSessions[socket.id].sendMessage([errorResult]);
                             io.emit('chat_message', { 
                                sender: NPC_NAME, 
                                text: completion.response.text(), 
                                color: "#00ffff" 
