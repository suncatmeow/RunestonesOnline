              ...
              setTimeout(async () => {
                  try {
                      ...
                      if (call) {
                          ...
                          // 1. Define the limit (Same as your client: 45)
                          const MAX_LEN = 45; 
                          let chunks = [];
                          let words = response.split(" ");
                          let currentLine = "";

                          // 2. Break the long AI response into lines
                          words.forEach(word => {
                          ...
                          // 3. Emit each line as a separate chat bubble
                          chunks.forEach(chunk => {
                              io.emit('chat_message', {
                                  sender: NPC_NAME,
                                  text: chunk,
                                  color: "#00ffff" // Cyan for Suncat
                              });
                          });
                      }
