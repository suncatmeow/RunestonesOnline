  ...
  socket.on('chat_message', async (msgText) => {
      ...
      if ((mentioned || isReply || (greeting && randomChance) || randomChance) && !npcIsTyping) {
          ...
          try {
              ...
              let suncatStatus = suncat ? `My Location: Map ${suncat.mapID}, Coords (${Math.floor(suncat.x)}, ${Math.floor(suncat.y)})` : "Status: Glitching";
  ...
  socket.on("suncat_spectate", async (actionDescription) => {
     ...
     //if (!suncat || !sender || suncat.mapID !== sender.mapID) return;
      ...
      try {
          ...
          TASK: Refer to players by name and generate a witty, short reaction (under 9 words).
          TONE: If they failed or something bad happened to them, mock them. If they did something impressive or succedded at something, be impressed or wary.
          `;
