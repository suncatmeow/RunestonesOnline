                      ...
                      if (call) {
                          ...
                          // [REPLACE YOUR EXISTING JUDGEMENT BLOCK WITH THIS]
                            ...
                            if (suncat && requester) {
                                ...
                                // [CRITICAL FIX] Broadcast the move IMMEDIATELY so the client updates
                                io.emit("updatePlayers", players);
                                
                                // 2. Lock onto this player so Suncat doesn't wander away in the next loop
                                currentTargetID = socket.id; 
                                lastSwitchTime = Date.now();

                                toolResultText = "Teleport successful. I am now standing next to the player.";
                            } else {
                                toolResultText = "Teleport failed. Could not find player coordinates.";
                            }

                            // 3. SEND THE RECEIPT TO THE AI BRAIN
                            const toolResult = {
                                functionResponse: {
                                    name: "teleportToPlayer",
                            ...
                            io.emit('chat_message', { 
                                sender: NPC_NAME, 
                                text: aiText || "*Suncat appears in a glitch of pixels.*", // Fallback if AI stays silent
                                color: "#00ffff" 
                            });
                        }
                      } else {
