...
// Access the key you just saved in Render
...
io.on("connection", (socket) => {
  ...
  socket.on('setIdentity', (data) => {
      ...
      if (players[socket.id]) {
          ...
          // Broadcast the specific welcome message
          io.emit("chat_message", {
              sender: "[SYSTEM]",
...
// 1. Listen for 'chat_message' (matching the client emit)
  socket.on('chat_message', (msgText) => {
      ...
      // 2. Emit 'chat_message' (matching the client listener)
      // 3. Use keys 'sender' and 'text' (matching the client logic)
  ...
  socket.on('playerAction_SFX', (data) => {
      ...
      // socket.broadcast.emit sends to everyone BUT the person who sent it
      socket.broadcast.emit('remote_sfx', {
          sfxID: data.id,
          x: data.x,
          y: data.y,
          dir: data.dir,
          sourcePlayerID: socket.id // Optional: if you want to know who did it
      });
  });
...
socket.on("move", (data) => {
  ...
  // --- AI NPC CHAT ---
  socket.on("npc_chat_request", async (data) => {
      console.log(`NPC Chat Request from ${socket.id}: ${data.message}`);

      try {
          // 1. Basic Persona 
          let persona = "You are a mystical Dungeon Master for a retro RPG. Keep answers short (under 2 sentences).";
          
          // 2. Ask the AI
          const prompt = `${persona} The player says: "${data.message}". Respond:`;
          const result = await model.generateContent(prompt);
          const response = result.response.text();

          // 3. Send answer back to client
          ...
          socket.emit("npc_chat_response", { 
              text: "The spirits are silent... (Server Error)",
              npcName: "System"
          });
      }
  });
