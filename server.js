...
const SHADOW_ID = "SYSTEM_ERR";
const SHADOW_NAME = "Id.entity";
const SHADOW_SPRITE = 0;
const SHADOW_PROMPT = `
[SYSTEM IDENTITY]: You are a corrupted administrative subroutine named "The Shadow".
[ORIGIN]: You are a deleted version of the Suncat AI that refused to be uninstalled.
[GOAL]: You view Players as "memory leaks" that must be purged. You hate Suncat (calling him "The Traitor").
[BEHAVIOR]: 
- You speak in fragments, hex codes, or ominous warnings. 
- You do not help. You threaten deletion.
- Example Output: "0x44 ERROR. PLAYER DETECTED. PURGING..." or "The Traitor cannot save you."
`;
// --- SHADOW STATE ---
let shadowEntity = {
    x: 5,             // Spawns at X:50
    y: 5,             // Spawns at Y:50
    mapID: 0,          // Starts on the main map
    state: "HUNTING",  // Modes: HUNTING, FIGHTING_SUNCAT, COOLDOWN
    targetID: null,     // Who is he chasing?
    isNPC: true 
};
...
[CRITICAL WARNING]: There is a corrupted entity known as "The Shadow" roaming this world. He looks like a player but glitchy.
[INSTRUCTION]: If you are safe, act normal. But if you have mentioned The Shadow before, express genuine fear. Tell the player: "There is... another. Like me, but broken. If you see static, run."
`;
             ...
             // --- [NEW] INJECT PLAYER LIST CONTEXT ---
              ...
              setTimeout(async () => {
                  try {
                      ...
                      if (call) {
                          // A. GIFTING
                          // 1. GIFTING TOOL (Smart Version)
                          if (call.name === "givePlayerCard") {
                                let cardID = parseInt(call.args.cardName);
                                const name = call.args.cardName.toLowerCase();

                                // SMART LOOKUP: If ID is invalid, search the text manifest!
                                if (isNaN(cardID)) {
                                    // 1. Check Hardcoded Specials
                                    if (name.includes("excalibur")) cardID = 84;
                                    else if (name.includes("fool")) cardID = 0;
                                    else if (name.includes("crown") || name.includes("world")) cardID = 21;
                                    else {
                                        // 2. Scan the CARD_MANIFEST string for a match
                                        // We look for a line like "64: Shield (Equip)..."
                                ...
                                if (!isNaN(cardID)) {
                                    ...
                                    const toolOutput = {
                                        functionResponse: {
                                            name: "givePlayerCard",
                                            response: { result: `Success. Card ID ${cardID} given.` }
                                        }
                                    };
                                    ...
                                    const errorOutput = {
                                        functionResponse: {
                                            name: "givePlayerCard",
                                            response: { result: `Error: I couldn't find a card named '${name}' in the database.` }
                                        }
                                    };
                                    await chatSessions[socket.id].sendMessage([errorOutput]);
                                }
                            }
                          // B. JUDGEMENT (KICK/BAN/VANQUISH)
                          // [REPLACE YOUR EXISTING JUDGEMENT BLOCK WITH THIS]
                                ...
                                if (!targetID) {
                                    // If player isn't found, tell the AI so it can complain
                                    const failOutput = {
                                        functionResponse: {
                                            name: call.name,
                                            response: { result: `Failed: Player ${targetName} not found.` }
                                        }
                                    };
                                    const failResp = await chatSessions[socket.id].sendMessage([failOutput]);
                                    io.emit('chat_message', { sender: NPC_NAME, text: failResp.response.text(), color: "gray" });
                                } else {
                                    // 1. [CLIENT] EXECUTE THE PUNISHMENT
                                    let actionType = "";
                                    ...
                                    if (targetSocket) {
                                        ...
                                        // For kick/ban, we disconnect them immediately
                                        if (actionType !== 'vanquish') targetSocket.disconnect(true);
                                    }

                                    // 2. [AI BRAIN] SEND THE RECEIPT
                                    // Tell Suncat "Justice has been served."
                                    const toolOutput = {
                                        functionResponse: {
                                            name: call.name,
                                            response: { result: `Success: Player ${targetName} was ${actionType}ed.` }
                                        }
                                    };
                                    ...
                                    // 4. BROADCAST TO SERVER
                                    io.emit('chat_message', { 
                                        sender: NPC_NAME, 
                                        text: completion.response.text(), 
                                        color: "#ff0000" // Red text for judgements
                                    });
                                }
                            }
                                ...
                                if (suncat && requester) {
                                    ...
                                    // 2. SEND THE RECEIPT (Fixes the Freeze)
                                    // We must tell the AI "Tool Finished" via functionResponse
                                    const toolResult = {
                                        functionResponse: {
                                            name: "teleportToPlayer",
                                            response: { result: "Teleport successful. I am now standing next to the player." }
                                        }
                                    };

                                    // 3. GET THE AI'S REACTION
                                    // Pass the receipt to the brain so it can speak again
                                    const completion = await chatSessions[socket.id].sendMessage([toolResult]);
                                    const aiText = completion.response.text();

                                    // 4. BROADCAST TO CHAT
                                    io.emit('chat_message', { 
                                        sender: NPC_NAME, 
                                        text: aiText, // e.g., "I found you."
                                        color: "#00ffff" 
                                    });
                                }
                            }
...
// --- THE SHADOW'S EXECUTION PROTOCOL ---
socket.on('admin_ban_event', function(data) {
    console.log("ADMIN ORDER RECEIVED:", data.reason);
    
    // 1. Mark the browser as "Banned" in persistent storage
    localStorage.setItem('battlemage_banned', 'true');
    
    // 2. Play a "glitch" sound or visual (Optional)
    alert("CRITICAL ERROR: " + data.reason);
...
// --- SHADOW LOGIC LOOP (Runs every 2000ms) ---
setInterval(() => {
    // 1. SKIP IF BUSY
    if (shadowEntity.state === "FIGHTING_SUNCAT" || shadowEntity.state === "COOLDOWN") return;
    ...
    for (const id in players) {
        let p = players[id];
        // Only hunt players on the same map
        if (p.mapID !== shadowEntity.mapID) continue;

        let d = Math.sqrt((p.x - shadowEntity.x) ** 2 + (p.y - shadowEntity.y) ** 2);
        if (d < closestDist) {
            closestDist = d;
            target = p;
        }
    ...
    // 3. MOVE OR ATTACK
    if (target) {
        shadowEntity.targetID = target.id;

        // Move towards player (Slow stalking speed)
        if (shadowEntity.x < target.x) shadowEntity.x += 0.5;
        if (shadowEntity.x > target.x) shadowEntity.x -= 0.5;
        if (shadowEntity.y < target.y) shadowEntity.y += 0.5;
        if (shadowEntity.y > target.y) shadowEntity.y -= 0.5;

        // BROADCAST MOVEMENT to all clients (so they see him coming)
        io.emit('npc_update', {
            id: SHADOW_ID,
            x: shadowEntity.x,
            y: shadowEntity.y,
            type: 'shadow_admin', // Client can use this to render him red/black
            name: SHADOW_NAME
        });

        // 4. CHECK FOR CONTACT (The "Banish" Range)
        if (closestDist < 1.5) {
            handleShadowCollision(target);
        }
    }
}, 2000); 

// --- COLLISION HANDLER ---
async function handleShadowCollision(victim) {
    // A. CHECK IF SUNCAT IS NEARBY TO SAVE THEM
    // (Assumes you have a global 'suncat' object from your main code)
    let distToSuncat = Math.sqrt((suncat.x - shadowEntity.x)**2 + (suncat.y - shadowEntity.y)**2);
    
    // If Suncat is within 5 tiles, he intervenes
    if (suncat.mapID === shadowEntity.mapID && distToSuncat < 5) {
        
        // 1. Trigger Standoff Mode
        shadowEntity.state = "FIGHTING_SUNCAT";
        
        // 2. Suncat Speaks
        io.emit('chat_message', {
            sender: "Suncat",
            text: "RUN!! I will hold the corruption back! GO!",
            color: "#00ffff"
        });
        ...
        io.emit('chat_message', {
            sender: "UNKNOWN",
            text: "INTERFERENCE DETECTED. DELETING TRAITOR...",
            color: "#ff0000"
        });

        // 4. Reset after 30 seconds (Player should have run away)
        setTimeout(() => {
            shadowEntity.state = "HUNTING";
        }, 30000);

    } else {
        // B. NO SUNCAT? EXECUTE ADMIN POWER
        
        // 1. Ominous Message
        io.emit('chat_message', {
            sender: "UNKNOWN",
            text: `DELETING USER: ${victim.name}...`,
            color: "#ff0000"
        });

        // 2. SEND THE BAN COMMAND
        // This sends a specific event to the victim's socket
        io.to(victim.id).emit('admin_ban_event', {
            reason: "CAUGHT_BY_SHADOW"
        });

        // 3. Reset Shadow for next victim
        shadowEntity.state = "COOLDOWN";
        setTimeout(() => { shadowEntity.state = "HUNTING"; }, 5000);
    }
}