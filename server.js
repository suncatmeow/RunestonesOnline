...
socket.on('chat_message', async (msgText) => {
    ...
    // 1. Broadcast to humans
    io.emit('chat_message', { sender: senderName, text: msgText, color: "white" });
    ...
    if (isTriggered && !npcIsTyping) {
        npcIsTyping = true; // Lock to prevent overlapping requests
        ...
        try {
            ...
            // --- CRITICAL FIX: HANDLE TOOL LOOPS ---
            // If the AI wants to run a function, we must run it AND send the result back
            // so it can generate the final text response.
            let functionCalls = result.response.functionCalls();
            
            while (functionCalls && functionCalls.length > 0) {
                const call = functionCalls[0]; // Get the first tool call
                
                // 1. Execute the tool logic
                const toolResult = await handleToolExecution(socket, call, senderName);
                
                // 2. Send the result back to Gemini ("Closing the loop")
                // The API *requires* this before it will give you text.
                ...
                // Check if it wants to run *another* tool or finally speak
                functionCalls = result.response.functionCalls();
            }
            // ----------------------------------------

            // Finally, emit the text response
            ...
            // If error is 400 (Bad Request), reset the brain because history is corrupted
