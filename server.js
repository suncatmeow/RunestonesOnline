...
io.on("connection", (socket) => {
  ...
  socket.on("join_game", (name) => {
      // 1. Update the name in server memory
      if (players[socket.id]) {
          players[socket.id].name = name;
      }
      
      // 2. Announce the name to everyone
      io.emit("chat_message", {
          sender: "[SYSTEM]",
          text: `${name} has entered the Pocket Plane.`
      });
      
      // 3. Force an update so everyone sees the new name on the sprite immediately
      io.emit("updatePlayers", players);
  });
...
socket.on("move", (data) => {
  ...
  socket.on("disconnect", () => {
    ...
    io.emit("chat_message", {
        sender: "[SYSTEM]",
        text: `Player ${socket.id.substr(0, 5)} has logged out.`
    });
