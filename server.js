...
// --- [NEW] SUNCAT BATTLE VOICE ---
  socket.on('suncat_battle_speak', async (data) => {
    // 1. We use the existing chat session so Suncat remembers the player
    const userSession = chatSessions[socket.id];
    
    // If no session exists (rare), we can't speak personally, so we skip
    if (!userSession) return;

    try {
        console.log("Suncat generating battle commentary...");

        // 2. Construct the prompt with the Game State + Manifest
        // We inject CARD_MANIFEST so he knows that "ID 26" is "Fire Spell"
        const battlePrompt = `
        [SYSTEM EVENT: BATTLE COMMENTARY]
        You are currently in a card duel with this player. 
        Analyze the battle state below and write a SHORT (under 12 words), glitchy, intense reaction.
        
        [CURRENT BATTLE STATE]
        - Phase: ${data.phase} (5=Planning/Select, 6=Dice Rolling, 7=Combat Resolution)
        - My Active Monster ID: ${data.myMonsterIDs.join(', ')} 
        - My Hand Card IDs: ${data.myHandIDs.join(', ')}
        - My Dice Roll: ${data.myRoll}
        - Enemy Dice Roll: ${data.enemyRoll}
        - Am I Winning the Roll?: ${data.winning}
        
        [REFERENCE DATA]
        ${CARD_MANIFEST}
        
        [INSTRUCTIONS]
        - If Phase 5 (Planning): Comment on the spell/monster you are using. Be mysterious.
        - If Phase 6/7 (Result): 
            - If you are WINNING (My Roll > Enemy Roll): Gloat arrogantly.
            - If you are LOSING: Glitch out, make excuses, or threaten them.
        - DO NOT include the phase number in your response. Just speak as Suncat.
        `;

        // 3. Send prompt to the AI
        // We use the session so he stays in character
        const result = await userSession.sendMessage(battlePrompt); 
        const response = result.response.text();

        // 4. Send the text back to the client to display
        io.emit('chat_message', {
            sender: "Suncat",
            text: response,
            color: "#00ffff" // Cyan text for Suncat
        });
